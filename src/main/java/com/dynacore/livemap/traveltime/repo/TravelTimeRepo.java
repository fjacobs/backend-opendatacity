...
public class TravelTimeRepo implements TrafficRepository<TravelTimeEntityImpl> {
  ...
  */
  ...
  public Flux<TravelTimeEntityImpl> getReplayData(OffsetDateTime start, Direction streamDirection) {
    Flux<TravelTimeEntityImpl> timeEntityFlux;
    char orderDirection;
    if (streamDirection == Direction.FORWARD) {
      ...
      timeEntityFlux =
          r2dbcClient
              .select()
              .from("travel_time_entity")
              .matching(where("pub_date").greaterThan(start))
              .orderBy(asc("pub_date"))
              .as(TravelTimeEntityImpl.class)
              .fetch()
              .all();
      ...
      timeEntityFlux =
          r2dbcClient
              .select()
              .from("travel_time_entity")
              .matching(where("pub_date").lessThan(start))
              .orderBy(desc("pub_date"))
              .as(TravelTimeEntityImpl.class)
              .fetch()
              .all();
    } else {
    ...
    //        return r2dbcClient
    //                .execute(
    //                        String.format("SELECT * FROM public.travel_time_entity WHERE pub_date
    // >= '%s' GROUP BY travel_time_entity.pub_date ORDER BY pub_date %s;", start, orderDirection))

    return timeEntityFlux;
  }
