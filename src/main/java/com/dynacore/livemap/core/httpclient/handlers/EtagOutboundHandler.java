...
public class EtagOutboundHandler extends ChannelOutboundHandlerAdapter {
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        log.debug("Assembling HTTP Request header: Read Etag: " + ctx.channel().attr(ChannelAttrKey.ETAG).get()
                                                               + " from channel: '" + ctx.channel().id() + "'");
        log.debug("Assembling HTTP Request header: Read 'Last-Modified' " + ctx.channel().attr(ChannelAttrKey.LAST_MODIFIED).get()
                                                               + " from channel " + ctx.channel().id());
